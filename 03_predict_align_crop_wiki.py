...
wiki_pd = pd.read_csv(aligned_wiki_csv, sep=';')
...
for age, age_category, gender, gender_id, path in zip(ages, age_categories, genders, gender_ids, image_paths):
    ...
    if(preds is not None):
        ...
        wiki_age_predict.append(age_index)
        wiki_gender_predict.append(gender_index)

        wiki_age.append(age)
        wiki_age_category.append(age_category)
        wiki_genders.append(gender)
        wiki_genders_id.append(gender_id)
        wiki_path.append(path)
        if age_category == age_index:
            wiki_age_result.append(True)
        else:
            wiki_age_result.append(False)
        if gender_id == gender_index:
            wiki_gender_result.append(True)
        else:
            wiki_gender_result.append(False)
    #     count+=1
    # if count==100:
    #     break
...
final_wiki = np.vstack((wiki_age, wiki_age_category, wiki_genders, wiki_genders_id, wiki_path, wiki_age_predict, wiki_gender_predict, wiki_age_result, wiki_gender_result)).T

final_wiki_df = pd.DataFrame(final_wiki)

final_wiki_df.columns = cols

final_wiki_df.to_csv(final_wiki_csv, index=False, sep=";")